...
function getTableName($tableNamePrefix) {
    return str_replace(" ", "_", strtolower($tableNamePrefix)."_results");
}
...
function getAvgPercentage_All($tableNamePrefix, $startDate, $endDate, $environment1, $groupName1, $environment2, $groupName2, $environment3, $groupName3, $isVerticalDataActive) {
    global $DB, $denominator;
    $jsonArray = array();
    ...
    for($i=1; $i<=3; $i++)
    {
        ...
        $sql = "select environment,groupName,Floor(sum(totalCases)) as totalCases,Floor(sum(passedCases)) as passedCases,Floor(sum(passedCases)*100/sum(totalCases)) as percentage from (
            SELECT environment, groupName, projectName, Floor(avg(totalCases)) as totalCases,Floor(avg(passedCases)) as passedCases
            FROM (
                     SELECT *,
                            ROW_NUMBER() OVER (PARTITION BY projectName ORDER BY percentage DESC) AS row_num
                     FROM " . getTableName($tableNamePrefix) . "
                     WHERE (groupName = '" . $groupName . "' AND environment = '" . $environment . "')  
                       AND date(createdAt) >= '" . $startDate . "' 
                       AND date(createdAt) <= '" . $endDate . "' 
                       AND projectName not like 'Vertical%'
                 ) AS ranked_results
            WHERE row_num <= CEILING(DATEDIFF('" . $endDate . "', '" . $startDate . "') / $denominator)
            GROUP BY projectName
            ORDER BY projectName, percentage DESC) as x;";
...
function getAvgPercentage($tableNamePrefix, $startDate, $endDate, $environment, $groupName, $isVerticalDataActive) {
    global $DB, $denominator;
    $jsonArray = array();
    $sql = "SELECT environment, groupName, projectName, Floor(sum(passedCases) * 100 / sum(totalCases)) AS percentage
    FROM (
        SELECT *,
                ROW_NUMBER() OVER (PARTITION BY projectName ORDER BY percentage DESC) AS row_num
        FROM " . getTableName($tableNamePrefix) . "
    ...
    WHERE row_num <= CEILING(DATEDIFF('" . $endDate . "', '" . $startDate . "') / $denominator)
    GROUP BY projectName
    ORDER BY projectName, percentage DESC;";
    $sql = showVerticalLevelData($sql, $isVerticalDataActive);
...
function getAvgExecutionTime($tableNamePrefix, $startDate, $endDate, $environment, $groupName, $isVerticalDataActive) {
    global $DB, $denominator;
    $jsonArray = array();
    $sql = "SELECT environment, groupName, projectName, 
    Floor(avg(totalCases)) as totalCases,Floor(avg(passedCases)) as passedCases, round(avg(duration)/60,2) as duration
    FROM (
        SELECT *,
                ROW_NUMBER() OVER (PARTITION BY projectName ORDER BY percentage DESC) AS row_num
        FROM " . getTableName($tableNamePrefix) . "
...
function getLast7Records($tableNamePrefix, $projectName, $startDate, $endDate, $environment, $groupName) {
    global $DB;
    $jsonArray = array();
    $jsonArrayDataSet = array();
    $jsonArrayCategory = array();
    ...
    $sql = "(select id,buildTag,resultLink,percentage,Date(createdAt) as createdAt from ".getTableName($tableNamePrefix)." where projectName in (" . $projectName . ") and environment='".$environment."' and groupName='".$groupName."' and date(createdAt)>='" . $startDate . "' and date(createdAt)<='" . $endDate . "' order by createdAt desc) order by createdAt desc,buildTag desc";
...
function getAvgPercentage_Project($tableNamePrefix, $projectName, $startDate, $endDate, $environment1, $groupName1, $environment2, $groupName2, $environment3, $groupName3, $isVerticalDataActive) {
    global $DB, $denominator;
    $jsonArray = array();

    $projectNamesArray = explode(',', $projectName);
    for($i=1; $i<=3; $i++)
    {
        $groupName = ${"groupName$i"};
        $environment = ${"environment$i"};
        ...
        for($j=0;$j<count($projectNamesArray); $j++)
        {
            $sql = "select environment,groupName,Floor(sum(totalCases)) as totalCases,Floor(sum(passedCases)) as passedCases,Floor(sum(passedCases)*100/sum(totalCases)) as percentage from (
                SELECT environment, groupName, projectName, Floor(avg(totalCases)) as totalCases,Floor(avg(passedCases)) as passedCases
                FROM (
                         ...
                         WHERE (groupName = '" . $groupName . "' AND environment = '" . $environment . "')  
                           AND date(createdAt) >= '" . $startDate . "' 
                           AND date(createdAt) <= '" . $endDate . "' 
                           AND projectName in (" . $projectNamesArray[$j] . ")
                     ) AS ranked_results
                WHERE row_num <= CEILING(DATEDIFF('" . $endDate . "', '" . $startDate . "') / $denominator)
                ORDER BY percentage DESC) as x;";
        ...
        $jsonArraySubSet1 = array();
        $jsonArrayItem = array();
        $jsonArrayItem['passedCases'] = $passedCases;
        $jsonArrayItem['totalCases'] = $totalCases;
        $jsonArrayItem['percentage'] = ($totalCases != 0) ? floor(($passedCases / $totalCases) * 100) : 0;
        array_push($jsonArraySubSet1, $jsonArrayItem);
        array_push($jsonArray, array(
            $environment . "-" . $groupName => $jsonArraySubSet1
        ));
    }
    return $jsonArray;
}

function getDailyAvgPercentage_Project($tableNamePrefix, $projectName, $startDate, $endDate, $environment1, $groupName1, $environment2, $groupName2, $environment3, $groupName3) {
    global $DB;
    $jsonArray = array();
    $lastDate = '2010-01-01';
    $lastValueForEnv1 = 0;
    ...
    $sql = "SELECT DATE(createdAt) as createdAt, sum(passedCases)*100/sum(totalCases) as percentage, environment,groupName from (select createdAt,environment,groupName,avg(passedCases) as passedCases, avg(totalCases) as totalCases from ".getTableName($tableNamePrefix)." where ((groupName='".$groupName1."' and environment='".$environment1."') or (groupName='".$groupName2."' and environment='".$environment2."') or (groupName='".$groupName3."' and environment='".$environment3."')) and date(createdAt)>='" . $startDate . "' and date(createdAt)<='" . $endDate . "' and projectName in (" . $projectName . ") GROUP BY DATE(createdAt),groupName,environment,projectName) as x GROUP BY DATE(createdAt),groupName,environment;";
...
function getDailyAvgExecutionTime_Project($tableNamePrefix, $projectName, $startDate, $endDate, $environment1, $groupName1, $environment2, $groupName2, $environment3, $groupName3) {
    global $DB;
    $jsonArray = array();
    $lastDate = '2010-01-01';
    $lastValueForEnv1 = 0;
    ...
    $jsonArraySubSet2 = array();
    $jsonArraySubSet3 = array();
    $sql = "SELECT DATE(createdAt) as createdAt, round(sum(duration)/60,2) as duration, environment,groupName from (select createdAt,environment,groupName,avg(duration) as duration from ".getTableName($tableNamePrefix)." where ((groupName='".$groupName1."' and environment='".$environment1."') or (groupName='".$groupName2."' and environment='".$environment2."') or (groupName='".$groupName3."' and environment='".$environment3."')) and date(createdAt)>='" . $startDate . "' and date(createdAt)<='" . $endDate . "' and projectName in (" . $projectName . ") GROUP BY DATE(createdAt),groupName,environment,projectName) as x GROUP BY DATE(createdAt),groupName,environment;";
    $sql = updateGroupBy($sql, $startDate, $endDate);
...
function getTotalCasesGroupwise_Project($tableNamePrefix, $projectName, $startDate, $endDate, $environment1, $groupName1, $environment2, $groupName2, $environment3, $groupName3) {
    global $DB;
    $jsonArray = array();
    $lastDate = '2010-01-01';
    $lastValue1ForEnv1 = 0;
    ...
    $sql = "SELECT DATE(createdAt) as createdAt, Floor(sum(totalCases)) as totalCases, Floor(sum(passedCases)) as passedCases, groupName, environment from (select createdAt,environment,groupName,avg(totalCases) as totalCases,avg(passedCases) as passedCases from ".getTableName($tableNamePrefix)." where ((groupName='".$groupName1."' and environment='".$environment1."') or (groupName='".$groupName2."' and environment='".$environment2."') or (groupName='".$groupName3."' and environment='".$environment3."')) and date(createdAt)>='" . $startDate . "' and date(createdAt)<='" . $endDate . "' and projectName in (" . $projectName . ") GROUP BY DATE(createdAt),groupName,environment,projectName) as x GROUP BY DATE(createdAt),groupName,environment;";
    $sql = updateGroupBy($sql, $startDate, $endDate);
...
// Four-group version for projects like TreasuryTeam3 with 4 groups (apiCases, webCases, androidCases, iosCases)
function getAvgPercentage_All_FourGroups($entityName, $startDate, $endDate, $environment1, $groupName1, $environment2, $groupName2, $environment3, $groupName3, $environment4, $groupName4, $projectName, $isVerticalDataActive) {
    global $DB, $denominator;
    $jsonArray = array();
...
if (!isset($jsonArray['error']))
{
    switch ($_GET['functionname'])
    {
        ...
        case 'getTotalCasesGroupwise_Project':
            ...
            break;
        case 'getAvgPercentage_All_FourGroups':
            validateParams(13, $_GET['arguments']);
            $jsonArray = getAvgPercentage_All_FourGroups($_GET['arguments'][0], $_GET['arguments'][1], $_GET['arguments'][2], $_GET['arguments'][3], $_GET['arguments'][4], $_GET['arguments'][5], $_GET['arguments'][6], $_GET['arguments'][7], $_GET['arguments'][8], $_GET['arguments'][9], $_GET['arguments'][10], $_GET['arguments'][11], $_GET['arguments'][12]);
            break;
