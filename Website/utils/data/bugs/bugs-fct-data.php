...
function getFctBugsWhereClause()
{
    return "bugCategory = 'FCT'";
}
...
function getProjectNames($tableNamePrefix, $startDate, $endDate, $isVerticalDataActive)
{
    global $DB;
    $jsonArray = array();
    $sql = "Select distinct projectName from " . getTableName($tableNamePrefix) . " WHERE " . getFctBugsWhereClause() . " order by projectName desc;";
...
function getTotalBugs($tableNamePrefix, $startDate, $endDate, $isVerticalDataActive)
{
    global $DB;
    $jsonArray = array();
    $sql = "SELECT IFNULL(c.category, 'TotalBugs') AS category, COUNT(a.classification) AS bugs FROM (SELECT 'PaymentGateway' AS category UNION ALL SELECT 'Invalid' UNION ALL SELECT 'Partner' UNION ALL SELECT 'TotalBugs') c LEFT JOIN " . getTableName($tableNamePrefix) . " a ON c.category = a.classification AND date(a.createdAt) >= '" . $startDate . "' AND date(a.createdAt) <= '" . $endDate . "' AND a.isDeleted=0 AND " . getFctBugsWhereClause() . " GROUP BY c.category WITH ROLLUP;";
...
function getBugCountData($tableNamePrefix, $startDate, $endDate, $isVerticalDataActive)
{
    ...
    $whereClause = buildCommonWhereClause($startDate, $endDate, $isVerticalDataActive, array(getFctBugsWhereClause()), 'createdAt');
...
function getTotalBugs_Project($tableNamePrefix, $startDate, $endDate, $projectName)
{
    ...
    $jsonArray = array();
    $whereClause = buildCommonWhereClause($startDate, $endDate, false, array($projectNameReference . " in (" . $projectName . ")", getFctBugsWhereClause()), 'createdAt');
    ...
    $sql = "SELECT priority, category, COUNT(*) AS totalRows, ROUND((SUM(isWithinSLA = 'Yes') / (SUM(isWithinSLA = 'Yes') + SUM(isWithinSLA = 'No'))) * 100, 0) AS slaPercentage 
            FROM (
                SELECT 
                    ...
                    END AS isWithinSLA 
                FROM " . getTableName($tableNamePrefix) . " 
                WHERE isDeleted=0 AND " . $projectNameReference . " in (" . $projectName . ") AND date(createdAt) >= '" . $startDate . "' AND date(createdAt) <= '" . $endDate . "' AND " . getFctBugsWhereClause() . "
            ) AS subquery 
            GROUP BY category, priority 
            ORDER BY priority, category ASC;";
...
function getBugCountTrendByPriority_Project($tableNamePrefix, $startDate, $endDate, $projectName, $category)
{
    ...
    $whereClause = buildCommonWhereClause($startDate, $endDate, false, array($projectNameReference . " IN (" . $projectName . ")", "classification IN ('" . str_replace(",", "','", $category) . "')", getFctBugsWhereClause()), 'createdAt');
