...
function getProdBugsWhereClause()
{
    return "bugCategory = 'PRD'";
}
...
$p2SlaInHours = 144;
$p3SlaInHours = 1440;
$firstReview_p0SlaInHours = 0.5;
$firstReview_p1SlaInHours = 2.5;
$firstReview_p2SlaInHours = 8;
$firstReview_p3SlaInHours = 25;
$firstReviewClosedStatuses = "'Pending PM','Ready for Development','Development In Progress','Ready for QA','QA Testing in Progress','Deployment Ready','Resolved/Closed','Closed - client coms done'";
$developmentClosedStatuses = "'Deployment Ready','Resolved/Closed','Closed - client coms done'";
$overallClosedStatuses = "'Resolved/Closed','Closed - client coms done'";
...
function getProjectNames($tableNamePrefix, $startDate, $endDate, $isVerticalDataActive)
{
    global $DB;
    $jsonArray = array();
    $sql = "Select distinct projectName from " . getTableName($tableNamePrefix) . " WHERE " . getProdBugsWhereClause() . " order by projectName desc;";
...
function getTotalBugs($tableNamePrefix, $startDate, $endDate, $isVerticalDataActive)
{
    global $DB;
    $jsonArray = array();
    $sql = "SELECT IFNULL(c.category, 'TotalBugs') AS category, COUNT(a.classification) AS bugs FROM (SELECT 'PaymentGateway' AS category UNION ALL SELECT 'Others' UNION ALL SELECT 'Invalid' UNION ALL SELECT 'TotalBugs') c LEFT JOIN " . getTableName($tableNamePrefix) . " a ON c.category = a.classification AND date(a.createdAt) >= '" . $startDate . "' AND date(a.createdAt) <= '" . $endDate . "' AND a.isDeleted=0 AND " . getProdBugsWhereClause() . " GROUP BY c.category WITH ROLLUP;";
...
function getBugCountData($tableNamePrefix, $startDate, $endDate, $isVerticalDataActive)
{
    ...
    $whereClause = buildCommonWhereClause($startDate, $endDate, $isVerticalDataActive, array(getProdBugsWhereClause()), 'createdAt');
    ...
    foreach ($rows as $row) {
        ...
        if ($previousPriority != $row["priority"]) {
                ...
                if ($row["priority"] == $singleRow["priority"]) {
                    switch ($singleRow['category']) {
                        ...
                        case "Invalid":
                            ...
                            break;
                        case "Others":
                            $increasedOthersBugs = $increasedOthersBugs + $singleRow['bugs'];
                            break;
                    }
                }
            }
            ...
            $jsonArrayItem2['value'] = $increasedinvalidBugs;
            array_push($jsonArrayBugs2, $jsonArrayItem2);

            $jsonArrayItem3['value'] = $increasedOthersBugs;
...
{
    global $DB, $p0SlaInHours, $p1SlaInHours, $p2SlaInHours, $p3SlaInHours, $overallClosedStatuses;
    $jsonArray = array();
    $jsonArrayCategory = array();
    $jsonArraySubCategory = array();
    $jsonArrayDataSet1 = array();
    ...
    $sql = "SELECT priority, category, COUNT(*) AS totalRows, ROUND((SUM(isWithinSLA = 'Yes') / (SUM(isWithinSLA = 'Yes') + SUM(isWithinSLA = 'No'))) * 100, 0) AS slaPercentage 
            FROM (
                SELECT 
                    ...
                    CASE 
                        WHEN priority = 'P0-Critical' THEN IF(overallTime > (" . $p0SlaInHours . " * 3600), 'No', IF(overallTime <= (" . $p0SlaInHours . " * 3600) AND status IN ($overallClosedStatuses), 'Yes', 'TBD'))
                        WHEN priority = 'P1-High' THEN IF(overallTime > (" . $p1SlaInHours . " * 3600), 'No', IF(overallTime <= (" . $p1SlaInHours . " * 3600) AND status IN ($overallClosedStatuses), 'Yes', 'TBD'))
                        WHEN priority = 'P2-Medium' THEN IF(overallTime > (" . $p2SlaInHours . " * 3600), 'No', IF(overallTime <= (" . $p2SlaInHours . " * 3600) AND status IN ($overallClosedStatuses), 'Yes', 'TBD'))
                        WHEN priority = 'P3-Low' THEN IF(overallTime > (" . $p3SlaInHours . " * 3600), 'No', IF(overallTime <= (" . $p3SlaInHours . " * 3600) AND status IN ($overallClosedStatuses), 'Yes', 'TBD'))
                        ELSE 'TBD' 
                    END AS isWithinSLA 
                ...
                if ($row["priority"] == $singleRow["priority"]) {
                    switch ($singleRow['category']) {
                        ...
                        case "Invalid":
                            ...
                            break;
                        case "Others":
                            $increasedOthersBugs = $increasedOthersBugs + $singleRow['slaPercentage'];
                            break;
                    }
                }
            }
...
function getFirstReviewBugSlaData($tableNamePrefix, $startDate, $endDate, $isVerticalDataActive)
{
    global $DB, $p0SlaInHours, $p1SlaInHours, $p2SlaInHours, $p3SlaInHours, $developmentClosedStatuses;
    $jsonArray = array();
    $jsonArrayCategory = array();
    $jsonArraySubCategory = array();
    $jsonArrayDataSet1 = array();
    ...
    $sql = "SELECT priority, category, COUNT(*) AS totalRows, ROUND((SUM(isWithinSLA = 'Yes') / (SUM(isWithinSLA = 'Yes') + SUM(isWithinSLA = 'No'))) * 100, 0) AS slaPercentage 
            FROM (
                SELECT 
                    classification as category, priority, 
                    CASE 
                        WHEN priority = 'P0-Critical' THEN IF(developmentTime > (" . $p0SlaInHours . " * 3600), 'No', IF(developmentTime <= (" . $p0SlaInHours . " * 3600) AND status IN ($developmentClosedStatuses), 'Yes', 'TBD'))
                        WHEN priority = 'P1-High' THEN IF(developmentTime > (" . $p1SlaInHours . " * 3600), 'No', IF(developmentTime <= (" . $p1SlaInHours . " * 3600) AND status IN ($developmentClosedStatuses), 'Yes', 'TBD'))
                        WHEN priority = 'P2-Medium' THEN IF(developmentTime > (" . $p2SlaInHours . " * 3600), 'No', IF(developmentTime <= (" . $p2SlaInHours . " * 3600) AND status IN ($developmentClosedStatuses), 'Yes', 'TBD'))
                        WHEN priority = 'P3-Low' THEN IF(developmentTime > (" . $p3SlaInHours . " * 3600), 'No', IF(developmentTime <= (" . $p3SlaInHours . " * 3600) AND status IN ($developmentClosedStatuses), 'Yes', 'TBD'))
                        ELSE 'TBD' 
                    END AS isWithinSLA 
                FROM " . getTableName($tableNamePrefix) . " 
                WHERE " . $whereClause . "
...
function getTotalBugs_Project($tableNamePrefix, $startDate, $endDate, $projectName)
{
    ...
    $jsonArray = array();
    $whereClause = buildCommonWhereClause($startDate, $endDate, false, array($projectNameReference . " in (" . $projectName . ")", getProdBugsWhereClause()), 'createdAt');
    $sql = "SELECT IFNULL(c.category, 'TotalBugs') AS category, COUNT(a.classification) AS bugs FROM (SELECT 'PaymentGateway' AS category UNION ALL SELECT 'Others' UNION ALL SELECT 'Invalid' UNION ALL SELECT 'TotalBugs') c LEFT JOIN " . getTableName($tableNamePrefix) . " a ON c.category = a.classification AND a." . $whereClause . " GROUP BY c.category WITH ROLLUP;";
...
function getBugCountData_Project($tableNamePrefix, $startDate, $endDate, $projectName, $category)
{
    ...
    $visible = (strpos($category, "Others") !== false) ? "1" : "0";
    array_push(
        $jsonArrayDataSet1,
        array(
            "seriesname" => "Other Bugs",
    ...
    $sql = "SELECT priority, category, COUNT(*) AS totalRows, ROUND((SUM(isWithinSLA = 'Yes') / (SUM(isWithinSLA = 'Yes') + SUM(isWithinSLA = 'No'))) * 100, 0) AS slaPercentage 
            FROM (
                SELECT 
                    ...
                    END AS isWithinSLA 
                FROM " . getTableName($tableNamePrefix) . " 
                WHERE isDeleted=0 AND " . $projectNameReference . " in (" . $projectName . ") AND date(createdAt) >= '" . $startDate . "' AND date(createdAt) <= '" . $endDate . "' AND " . getProdBugsWhereClause() . "
            ) AS subquery 
            GROUP BY category, priority 
            ORDER BY priority, category ASC;";
...
function getFirstReviewBugSlaData_Project($tableNamePrefix, $startDate, $endDate, $projectName, $category)
{
    global $DB, $firstReview_p0SlaInHours, $firstReview_p1SlaInHours, $firstReview_p2SlaInHours, $firstReview_p3SlaInHours, $firstReviewClosedStatuses;
    $projectNameReference = getProjectNameReference($projectName);
    ...
    $sql = "SELECT priority, category, COUNT(*) AS totalRows, ROUND((SUM(isWithinSLA = 'Yes') / (SUM(isWithinSLA = 'Yes') + SUM(isWithinSLA = 'No'))) * 100, 0) AS slaPercentage 
            FROM (
                SELECT 
                    classification as category, priority, 
                    CASE 
                        WHEN priority = 'P0-Critical' THEN IF(pmTime > (" . $firstReview_p0SlaInHours . " * 3600), 'No', IF(pmTime <= (" . $firstReview_p0SlaInHours . " * 3600) AND status IN ($firstReviewClosedStatuses), 'Yes', 'TBD'))
                        WHEN priority = 'P1-High' THEN IF(pmTime > (" . $firstReview_p1SlaInHours . " * 3600), 'No', IF(pmTime <= (" . $firstReview_p1SlaInHours . " * 3600) AND status IN ($firstReviewClosedStatuses), 'Yes', 'TBD'))
                        WHEN priority = 'P2-Medium' THEN IF(pmTime > (" . $firstReview_p2SlaInHours . " * 3600), 'No', IF(pmTime <= (" . $firstReview_p2SlaInHours . " * 3600) AND status IN ($firstReviewClosedStatuses), 'Yes', 'TBD'))
                        WHEN priority = 'P3-Low' THEN IF(pmTime > (" . $firstReview_p3SlaInHours . " * 3600), 'No', IF(pmTime <= (" . $firstReview_p3SlaInHours . " * 3600) AND status IN ($firstReviewClosedStatuses), 'Yes', 'TBD'))
                        ELSE 'TBD' 
                    END AS isWithinSLA 
                FROM " . getTableName($tableNamePrefix) . " 
                WHERE isDeleted=0 AND " . $projectNameReference . " in (" . $projectName . ") AND date(createdAt) >= '" . $startDate . "' AND date(createdAt) <= '" . $endDate . "' AND " . getProdBugsWhereClause() . "
...
function getIssuesList_Project($tableNamePrefix, $startDate, $endDate, $projectName, $category)
{
    ...
    $jsonArray = array();
    $sql = "SELECT issueId, createdAt, title,priority, teamName, bugType, rootCause, environment, status,
                CONCAT(CASE WHEN qaTime = 0 THEN '0' ELSE CONCAT(ROUND(qaTime/3600, 1), 'H') END) AS qaTimeHours, 
                CASE 
                    WHEN priority = 'P0-Critical' THEN IF(qaTime > (" . $firstReview_p0SlaInHours . " * 3600), 'No', IF(qaTime <= (" . $firstReview_p0SlaInHours . " * 3600) AND status IN ($firstReviewClosedStatuses), 'Yes', 'TBD')) 
                    WHEN priority = 'P1-High' THEN IF(qaTime > (" . $firstReview_p1SlaInHours . " * 3600), 'No', IF(qaTime <= (" . $firstReview_p1SlaInHours . " * 3600) AND status IN ($firstReviewClosedStatuses), 'Yes', 'TBD')) 
                    WHEN priority = 'P2-Medium' THEN IF(qaTime > (" . $firstReview_p2SlaInHours . " * 3600), 'No', IF(qaTime <= (" . $firstReview_p2SlaInHours . " * 3600) AND status IN ($firstReviewClosedStatuses), 'Yes', 'TBD')) 
                    WHEN priority = 'P3-Low' THEN IF(qaTime > (" . $firstReview_p3SlaInHours . " * 3600), 'No', IF(qaTime <= (" . $firstReview_p3SlaInHours . " * 3600) AND status IN ($firstReviewClosedStatuses), 'Yes', 'TBD')) 
                    ELSE 'TBD' 
                END AS isWithinFirstReviewSLA,
                CONCAT(CASE WHEN developmentTime = 0 THEN '0' ELSE CONCAT(ROUND(developmentTime/3600, 1), 'H') END) AS developmentTimeHours,
...
function getBugCountTrendByPriority_Project($tableNamePrefix, $startDate, $endDate, $projectName, $category)
{
    ...
    $whereClause = buildCommonWhereClause($startDate, $endDate, false, array($projectNameReference . " IN (" . $projectName . ")", "classification IN ('" . str_replace(",", "','", $category) . "')", getProdBugsWhereClause()), 'createdAt');
...
if (!isset($jsonArray['error'])) {
    switch ($_GET['functionname']) {
        ...
        case 'getOverallBugSlaData':
            ...
            break;
        case 'getFirstReviewBugSlaData':
            validateParams(4, $_GET['arguments']);
            $jsonArray = getFirstReviewBugSlaData($_GET['arguments'][0], $_GET['arguments'][1], $_GET['arguments'][2], $_GET['arguments'][3]);
            break;
        ...
        case 'getOverallBugSlaData_Project':
            ...
            break;
        case 'getFirstReviewBugSlaData_Project':
            validateParams(5, $_GET['arguments']);
            $jsonArray = getFirstReviewBugSlaData_Project($_GET['arguments'][0], $_GET['arguments'][1], $_GET['arguments'][2], $_GET['arguments'][3], $_GET['arguments'][4]);
            break;
