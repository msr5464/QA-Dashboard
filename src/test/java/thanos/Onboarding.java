...
public class Onboarding extends TestBase {
	...
	String teamName = "Onboarding";

	@Test(priority = 1, dataProvider = "getTestConfig", description = "Fetch Only FCT testcases from the TestRail", groups = {
			"dataPopulator" })
	public void fetchFctTestcasesFromTestrail(Config testConfig) {
		...
		JSONObject testRailProjects = CommonUtilities
				.getJsonObjectFromJsonFile(getEntityConfigPath(teamName) + "TestRailConfig.json");
		testRailHelper.fetchDataFromTestRail(testConfig, testRailProjects, entityName, DataType.FctTests);
	}

	@Test(priority = 3, dataProvider = "getTestConfig", description = "Fetch All the testcases from the TestRail", groups = {
			"dataPopulator" })
	...
	@Test(priority = 2, dataProvider = "getTestConfig", description = "Fetch tickets tested and ALL bugs from Jira using unified approach", groups = {
			"dataPopulator" })
	public void fetchBugMetricsData(Config testConfig) {
		BugMetricsHelper bugMetricsHelper = new BugMetricsHelper(testConfig);
		JSONObject jiraProjects = CommonUtilities
				.getJsonObjectFromJsonFile(getEntityConfigPath(teamName) + "JiraConfig.json");
		...
		bugMetricsHelper.fetchDataForProjectsAndVerticals(testConfig, jiraProjects, entityName, LocalDate.now(),
				jiraFilters, false);

		testConfig.logComment("====================================================");
		testConfig.logComment("PART 2: Fetching Tickets Tested → paymentgateway_jira_tickets");
		testConfig.logComment("====================================================");
		testConfig.putRunTimeProperty("tableName2",
				entityName.toLowerCase().trim().replaceAll(" ", "_") + "_jira_tickets");
		bugMetricsHelper.fetchDataForProjectsAndVerticals(testConfig, jiraProjects, entityName, LocalDate.now(),
				jiraFilters, true);
	}

	@Test(priority = 4, dataProvider = "getTestConfig", description = "Code for calculating Vertical level and Entity level data", groups = {
			"dataPopulator" })
	public void fetchAutomationStabilityData(Config testConfig) {
		...
		for (LocalDate date = startDate; date.isBefore(endDate.plusDays(1)); date = date.plusDays(1)) {
			JSONObject automationProjects = CommonUtilities
					.getJsonObjectFromJsonFile(getEntityConfigPath(teamName) + "AutomationConfig.json");
			resultsHelper.fetchAndUpdateResultsData(testConfig, entityName, automationProjects,
					environmentAndGroupNamePairs, date);
		}
	}
}